...
@property (weak, nonatomic) DMUserSurveyRootForm *userSurveyRoot;  // Modified by MUKAI Takeshi in 2015-10
@end
...
- (NSSet*)incompleteFieldKeys {
    ...
    if ((NSInteger)self.userSurveyRoot.userSurvey.sex == -1) {
        [incompleteFields addObjectsFromArray:DM_SEX_OPTIONS];
    }
    if ((NSInteger)self.userSurveyRoot.userSurvey.ageBracket == -1) {
        [incompleteFields addObjectsFromArray:DM_AGE_OPTIONS];
    }
    if ((NSInteger)self.userSurveyRoot.userSurvey.livingArrangement == -1) {
        [incompleteFields addObjectsFromArray:DM_LIVING_ARRANGEMENT_OPTIONS];
    }
